...
const taskQueue = new TranscriptQueue(10);
...
(async () => {
  ...
  const selected = await inquirer.prompt([
    {
      ...
      name: 'language',
      message: '请选择语言',
      choices: ['all', ...res.map(v => v.label)],
    }
  ]);
  let language: any = {};
  const run = async (language: any) => {
    subscribing = true;
    if (language) {
  ...
  if (selected.language === 'all') {
    for (const lan of res) {
      await run(lan);
    }
  } else {
    language = res.find(v => v.label === selected.language);
    await run(language);
  }
